...
echo "Instantiating smart contract on ucpchannel"
docker exec \
  -e CORE_PEER_LOCALMSPID=Org1MSP \
  -e CORE_PEER_MSPCONFIGPATH=${ORG1_MSPCONFIGPATH} \
  cli \
...
Next, use the remittancecc applications to interact with the deployed remittancecc contract.
The remittancecc applications are available in multiple programming languages.
